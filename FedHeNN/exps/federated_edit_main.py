...
from update import GlobalRepGenerator, LocalUpdate, save_protos, LocalTest, test_inference_new_het_lt, test_inference_new_het_lt_edit
...
def FedProto_taskheter(args, train_dataset, test_dataset, user_groups, user_groups_lt, local_model_list, classes_list):
    ...
    for round in tqdm(range(args.rounds)):
        ...
        # RAD Generator: random sampling
        RAD_loader = GlobalRepGenerator(train_dataset)
        unlabeled_dataset = [data[0] for data in RAD_loader]
        images_RAD = random.sample(unlabeled_dataset, args.local_bs)

        # Get all the reps from clients
        for idx in idxs_users:
            _, reps = local_model_list[idx](images_RAD)
            # global_reps.append(copy.deepcopy(reps.data))
            # caculate the Kj
            local_k.append(torch.dot(reps, reps))

        

        for idx in idxs_users:
            local_model = LocalUpdate(local_k, args=args, dataset=train_dataset, idxs=user_groups[idx])
            w, loss, acc, protos = local_model.update_weights_het(images_RAD, local_k, args, idx, model=copy.deepcopy(local_model_list[idx]), global_round=round)
            # 根据平均值得到本地原型
        ...
        # ---------------------
        acc_list, loss1_list, loss2_list, total_loss_list =  test_inference_new_het_lt_edit(args, t_local_model_list, test_dataset, classes_list, user_groups_lt, global_k, )
...
def FedProto_modelheter(args, train_dataset, test_dataset, user_groups, user_groups_lt, local_model_list, classes_list):
    ...
    for round in tqdm(range(args.rounds)):
        ...
        # RAD Generator: random sampling
        RAD_Generator = GlobalRepGenerator(args=args, dataset=train_dataset)
        RAD_loader = RAD_Generator.dataloader
        images_RAD, _ = next(iter(RAD_loader))
        images_RAD = images_RAD.to(args.device)
        ...
        for idx in idxs_users:
            ...
            # global_reps.append(copy.deepcopy(reps.data))
            # caculate the Kj
            local_ks.append(torch.matmul(reps, reps.T))

        global_k.resize_(local_ks[0].data.size())
        global_k = global_k.to(args.device)

        for i in local_ks:
            global_k += i.data
        global_k = global_k / torch.tensor(float(len(local_ks)), dtype=torch.float32)

        for idx in idxs_users:
            local_model = LocalUpdate(args=args, dataset=train_dataset, idxs=user_groups[idx])
            w, loss, acc = local_model.update_weights_het(images_RAD, global_k, args, idx, model=copy.deepcopy(local_model_list[idx]), global_round=round)
    ...
    acc_list_g = test_inference_new_het_lt_edit(args, local_model_list, test_dataset, user_groups_lt, global_k, images_RAD)
    print('For all users (with protos), mean of test acc is {:.5f}, std of test acc is {:.5f}'.format(np.mean(acc_list_g),np.std(acc_list_g)))
    # print('For all users (w/o protos), mean of test acc is {:.5f}, std of test acc is {:.5f}'.format(np.mean(acc_list_l), np.std(acc_list_l)))
