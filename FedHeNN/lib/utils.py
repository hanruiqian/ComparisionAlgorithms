...
def loss_CKA(local_k, global_k):

    local_k_cpu = local_k.cpu()
    global_k_cpu = global_k.cpu()

    local_k_cpu_detached = local_k_cpu.detach()
    global_k_cpu_detached = global_k_cpu.detach()


    # Compute the trace of the product of the matrices
    tr_Ai_Aj = np.trace(local_k_cpu_detached.T @ global_k_cpu_detached)
    tr_Ai_Ai = np.trace(local_k_cpu_detached.T @ local_k_cpu_detached)
    tr_Aj_Aj = np.trace(global_k_cpu_detached.T @ global_k_cpu_detached)

    # Compute the linear CKA
    linear_cka = (tr_Ai_Aj ** 2) / (tr_Ai_Ai * tr_Aj_Aj)
    # print (linear_cka)
    return linear_cka
