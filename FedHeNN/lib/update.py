...
class GlobalRepGenerator(object):
    ...
    def RAD_loader(self, dataset):
        ...
        """
        dataloader = DataLoader(dataset, batch_size=self.args.local_bs,
...
class LocalUpdate(object):
    ...
    def update_weights_prox(self, idx, local_weights, model, global_round):
        ...
        return model.state_dict(), sum(epoch_loss) / len(epoch_loss), acc_val.item()
    # -----------
    def update_weights_het(self, images_RAD, global_k, args, idx, model, global_round=round):
        # Set mode to train model
        model.train()
        ...
        for iter in range(self.args.train_ep):
            ...
            for batch_idx, (images, label_g) in enumerate(self.trainloader):
                ...
                model.zero_grad()
                # 前向传播得出一个batch的softmaxes
                log_probs, protos = model(images)
                # 求交叉熵loss1
                loss1 = self.criterion(log_probs, labels)

                _, reps = model(images_RAD)
                local_k = torch.matmul(reps, reps.T)

                if len(global_k) == 0:
                    loss2 = 0*loss1
                else:
                    loss2 = loss_CKA(local_k, global_k)
...
def test_inference_new_het_lt(args, local_model_list, test_dataset, classes_list, user_groups_gt, global_k, RAD_loader):
    """ Returns the test accuracy and loss.
    """
    loss, total, correct = 0.0, 0.0, 0.0
    loss_mse = nn.MSELoss()
    ...
    device = args.device
    criterion = nn.NLLLoss().to(device)
    # 所有客户端在测试集上使用神经网络和prototype的准确率加权（不准）
    acc_list_g = []
    ...
    for idx in range(args.num_users):
        ...
        # correct和total在用proto预测前应该清零，作者没清零所以该proto结果是建立在神经网络预测后的加权结果
        ...
        if global_k!=[]:
            for batch_idx, (images, labels) in enumerate(testloader):
                ...
                pred_labels = pred_labels.view(-1)
                # 这个correct和total在用proto预测前是不是应该清零啊
                correct += torch.sum(torch.eq(pred_labels, labels)).item()
                total += len(labels)
...
def test_inference_new_het_lt_edit(args, local_model_list, test_dataset, user_groups_gt, global_k, images_RAD):
    """ Returns the test accuracy and loss.
    """
    loss, total, correct = 0.0, 0.0, 0.0
    loss_mse = nn.MSELoss()
    ...
    for idx in range(args.num_users):
        ...
        model = local_model_list[idx]
        model.to(args.device)
        testloader = DataLoader(DatasetSplit(test_dataset, user_groups_gt[idx]), batch_size=32, shuffle=True)
        # test (local model)
        model.eval()
        for batch_idx, (images, labels) in enumerate(testloader):
            images, labels = images.to(device), labels.to(device)
            ...
            #     i += 1
            # loss_mse = nn.MSELoss()
            # loss2_1 = loss_mse(proto_new, protos)
            # if args.device == 'cuda':
            #     loss2_1 = loss2_1.cpu().detach().numpy()
            # else:
            #     loss2_1 = loss2_1.detach().numpy()
            # loss2 += loss2_1
            batch_total += 1
            total += len(labels)
        # 客户端i的最终准确率
        ...
        # 客户端i的Loss1
        loss1 = loss / batch_total
        print('| User: {} | Test Loss1: {:.3f}'.format(idx, loss1))
        loss1_list.append(loss1)

        # 客户端i的Loss2
        loss2 = loss2 / batch_total
        print('| User: {} | Test Loss2: {:.3f}'.format(idx, loss2))
        loss2_list.append(loss2)

        # 客户端i的Total Loss
        total_loss = loss1 + loss2 * args.ld
        print('| User: {} | Test Total Loss: {:.3f}'.format(idx, total_loss))
        total_loss_list.append(total_loss)
    # return acc_list, loss1_list, loss2_list, total_loss_list
    return acc_list
